    ...
    return(
        <div className='w-full max-w-5xl  h-full min-h-full mx-auto bg-white rounded-md shadow-xl flex md:flex-col items-center py-4 overflow-hidden'>
            <div className="w-2/3 bg-white rounded p-8 m-4 items-center">
                <h1 className="w-full text-center text-orange-400 text-2xl font-bold mb-8">Compra finalizada exitosamente</h1>
                    <div className="flex flex-col items-center ">
                        <p className="md:mb-1  md:py-4 md:px-6 flex-auto text-lg text-gray-600"> Estimado(a) {name} {lastName} </p> 
                        <p className="md:mb-1  md:py-4 md:px-6 flex-auto text-lg text-gray-600"> Su compra se ha completado de manera satisfactoria, le sugerimos que guarde su número de compra para seguimiento del despacho. </p>    
                ...
                <Link to="/"><button className="block bg-orange-300 hover:bg-orange-400 text-white font-bold text-lg mx-auto p-3 py-3 rounded-md" type="submit">Volver a la tienda</button></Link>           
            </div>
        </div>
    )
}
